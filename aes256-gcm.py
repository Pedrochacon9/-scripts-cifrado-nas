...
def generar_guardar_clave():
    ...
    nonce = os.urandom(12)  # GCM usa un nonce de 12 bytes
    cipher = Cipher(algorithms.AES(clave_cifrado), modes.GCM(nonce), backend=default_backend())
    ...
    with open("clave_maestra_cifrada_gcm.key", "wb") as f:
        f.write(salt + nonce + tag + clave_maestra_cifrada)

    print("Clave maestra cifrada con AES-256-GCM y almacenada correctamente.")
